...
<body class="container-fluid bg-primary">
    ...
    $(document).ready(function() {
        fetchDeptFacultyMembers(0, 10000000);
        // FETCH ALL MajorS
        function fetchDeptFacultyMembers(start, limit) {
            $.ajax({
                url: baseUrl + '/app/controllers/query/fetchDataTables.php',
                method: 'POST',
                dataType: 'text',
                data: {
                    fetchDeptFacultyMembers: 'fetchDeptFacultyMembers',
                    start: start,
                    limit: limit
                },
                success: function(response) {
                    if (response != "reachedMax") {
                        ...
                        start += limit;
                        fetchDeptFacultyMembers(start, limit);
                    } else {
                        $("#FacultyMembersTable").DataTable({
                            ...
                            buttons: [{
                                ...
                                extend: 'pdfHtml5',
                                // download: 'open',
                                title: 'Department Faculty Members',
                                exportOptions: {
                                    columns: [0, 1, 2, 3]
                                }
                            }],
